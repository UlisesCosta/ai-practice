...
def entrenar_knn(df: pd.DataFrame):
    ...
    grid = GridSearchCV(
        pipe, parametros, cv=5, n_jobs=-1, scoring="accuracy"
    )
...
df = cargar_csv("../outputs_csvs/afectacion.csv")
grid, X_test, y_test = entrenar_knn(df)
matriz = obtener_matriz_confusion(grid, X_test, y_test)
graficar_matriz_confusion(matriz, "Matriz de Confusión Afectacion")